...
entt::entity CameraComponentApi::getCurrentCamera() const {
    return voxlight.currentCamera;
}

void CameraComponentApi::setCurrentCamera(entt::entity camera) {
    voxlight.currentCamera = camera;
}

glm::mat4 CameraComponentApi::getViewProjectionMatrix() const {
    auto [cameraComponent, transformComponent] = voxlight.registry.get<CameraComponent, TransformComponent>(voxlight.currentCamera);
    auto cameraView = glm::lookAt(transformComponent.position, transformComponent.position + cameraComponent.direction, {0.f, 1.f, 0.f});
