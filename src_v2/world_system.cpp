...
static inline std::uint32_t idx(glm::ivec3 pos) {
  auto pos0 = pos >> 1;
  return pos0.x + pos0.y * HalfWorldSize + pos0.z * HalfWorldSize * HalfWorldSize;
}
...
void WorldSystem::update(float) {
  ...
  engine->getRegistry().sort<VoxelComponent>([](auto const &a, auto const &b) { return a.distance > b.distance; });
}

void WorldSystem::deinit() {}

void WorldSystem::setVoxel(glm::ivec3 position) { voxelMap[idx(position)] |= bitMask(position); }

void WorldSystem::clearVoxel(glm::ivec3 position) { voxelMap[idx(position)] &= ~bitMask(position); }

std::uint32_t WorldSystem::createVoxelEntity(VoxelData const &data, glm::vec3 position, glm::quat rotation) {
  auto texId = engine->getRenderSystem().createVoxelTexture(data.getDataAsVector(), data.getDimensions());
  auto voxelEntity = engine->getRegistry().create();
  engine->getRegistry().emplace<VoxelComponent>(voxelEntity, texId, true, 0.f);
  engine->getRegistry().emplace<TransformComponent>(voxelEntity, position, glm::vec3(data.getDimensions()), rotation);
