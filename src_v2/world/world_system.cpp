...
void WorldSystem::init() {
    ...
    getEngine()->renderSystem.createWorldTexture(voxelMap, glm::ivec3(HalfWorldSize));
}
...
void WorldSystem::update(float) {
    ...
    auto cameraPos = getEngine()->controllerSystem.getCameraPosition();
...
std::uint32_t WorldSystem::createVoxelEntity(VoxelData<std::uint8_t> const& data, glm::vec3 position,
    ...
    auto texId = getEngine()->renderSystem.createVoxelTexture(data.getDataAsVector(), size);
    auto voxelEntity = getEngine()->getRegistry().create();
    getEngine()->getRegistry().emplace<VoxelComponent>(voxelEntity, texId, true, 0.f);
    ...
    // getEngine()->worldSystem()->updateWorldTexture(data, size,
    //                                                            glm::ivec3(HalfWorldSize));  // todo: temporary
    //                                                            solution
    return 0;
}