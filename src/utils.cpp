...
VoxelContainer loadVox(std::filesystem::path path) {
    ...
    auto readInt = [](std::istreambuf_iterator<char>& iterator) {
        ...
        return static_cast<std::int32_t>(data[0] | data[1] << 8 |
                                         data[2] << 16 | data[3] << 24);
    };
    ...
    if (fs::exists(path) && path.extension() == ".vox") {
        ...
        while (iterator != std::istreambuf_iterator<char>()) {
            ...
            if (tag == "SIZE") {
                ...
                container.data.resize(container.size.x * container.size.y *
                                      container.size.z);
            } else if (tag == "XYZI") {
                ...
                for (auto i = 0; i < voxelCount; i++) {
                    ...
                    std::advance(iterator, 1);
                    container.data[x + y * container.size.x +
                                   z * container.size.x * container.size.y] =
                        colorIndex;
                }
            } else {
