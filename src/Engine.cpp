...
Engine::Engine()
    : voxelWorld(nullptr), camera(nullptr), windowWidth(1280),
      windowHeight(720) {
  glfwInit();
  ...
  window =
      glfwCreateWindow(windowWidth, windowHeight, "Voxlight", nullptr, nullptr);
  if (!window) {
...
void Engine::run() {
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    // to create a named window.
    {
      ...
      ImGui::Begin("Camera position"); // Create a window called "Hello, world!"
                                       // and append into it.
      ImGuiIO &io = ImGui::GetIO();
...
std::pair<std::uint32_t, std::uint32_t> Engine::getWindowSize() const {
