...
static const GLfloat quadBufferData[] = {-1.f, -1.f, 0.f, 1.f,  1.f,  0.f,
...
void VoxelSystem::initialise() {
    ...
    auto [vertexShader, vertexShaderStatus] =
        createShader(GL_VERTEX_SHADER, VOXEL_VERTEX_SHADER_SRC);
    auto [fragmentShader, fragmentShaderStatus] =
        createShader(GL_FRAGMENT_SHADER, VOXEL_FRAGMENT_SHADER_SRC);
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(cubeBufferData), cubeBufferData,
                 GL_STATIC_DRAW);
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB_SNORM, WINDOW_WIDTH, WINDOW_HEIGHT, 0,
                 GL_RGB, GL_FLOAT, 0);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
    ...
    glGenTextures(1, &depthTexture);
    glBindTexture(GL_TEXTURE_2D, depthTexture);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_R32F, WINDOW_WIDTH, WINDOW_HEIGHT, 0,
                 GL_RGB, GL_FLOAT, 0);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
...
void VoxelSystem::update(float deltaTime, Camera &camera) {
    ...
    auto sunPosition = glm::vec3(glm::sin(sunRotation) * 10000000, 10000000,
                                 glm::cos(sunRotation) * 10000000);
    ...
    glm::mat4 projection = glm::perspective(
        glm::radians(
            90.f),  // The vertical Field of View, in radians: the amount of
                    // "zoom". Think "camera lens". Usually between 90° (extra
                    // wide) and 30° (quite zoomed in)
        16.0f /
            9.0f,  // Aspect Ratio. Depends on the size of your window. Notice
    ...
    GLuint viewProjectionInvMatrixID =
        glGetUniformLocation(voxelProgram, "uViewProjectionInvMatrix");
    ...
    std::sort(
        ...
        [&voxelComponents, &camera](std::uint32_t a, std::uint32_t b) {
            auto closestPointA = glm::clamp(
                camera.getPosition(), voxelComponents[a].position,
                voxelComponents[a].position + voxelComponents[a].size);
            auto closestPointB = glm::clamp(
                camera.getPosition(), voxelComponents[b].position,
    ...
    glUniformMatrix4fv(viewProjectionMatrixID, 1, GL_FALSE,
                       &viewProjection[0][0]);
    glUniformMatrix4fv(viewProjectionInvMatrixID, 1, GL_FALSE,
                       &viewProjectionInv[0][0]);
    glUniform2f(glGetUniformLocation(voxelProgram, "invResolution"),
                1.f / WINDOW_WIDTH, 1.f / WINDOW_HEIGHT);
    ...
    for (auto entity : entities) {
        ...
        auto scale = glm::scale(glm::mat4(1.f),
                                voxelComponent.size * voxelComponent.voxelSize);
        auto model = trans * rot * scale;
        glUniformMatrix4fv(modelMatrixID, 1, GL_FALSE, &model[0][0]);
        auto minBox = voxelComponent.position;
        auto maxBox = minBox + voxelComponent.size;
        glUniform3f(glGetUniformLocation(voxelProgram, "uMinBox"), minBox.x,
                    minBox.y, minBox.z);
        glUniform3f(glGetUniformLocation(voxelProgram, "uMaxBox"), maxBox.x,
                    maxBox.y, maxBox.z);
        glUniform3f(glGetUniformLocation(voxelProgram, "uChunkSize"), size.x,
                    size.y, size.z);
        glUniform1f(glGetUniformLocation(voxelProgram, "uVoxSize"),
                    voxelComponent.voxelSize);
        glUniform3f(glGetUniformLocation(voxelProgram, "uSunPos"),
        ...
        glUniformMatrix4fv(glGetUniformLocation(voxelProgram, "uMagicMatrix"),
                           1, GL_FALSE, &worldMatrix[0][0]);

        glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_3D, voxelComponent.voxelTexture);

        glDrawArrays(GL_TRIANGLES, 0,
                     36);  // 3 indices starting at 0 -> 1 triangle
        // glDrawArrays(GL_LINES, 0, 36); // 3 indices starting at 0 -> 1
        // triangle
    ...
    glUniform2f(glGetUniformLocation(sunlightProgram, "invResolution"),
                1.f / WINDOW_WIDTH, 1.f / WINDOW_HEIGHT);
    glUniformMatrix4fv(glGetUniformLocation(sunlightProgram, "uMagicMatrix"), 1,
                       GL_FALSE, &mm[0][0]);
    glUniform3f(glGetUniformLocation(sunlightProgram, "uSunPos"), sunPosition.x,
    ...
    for (std::size_t x = 0; x < entityCompontent.size.x; ++x) {
        for (std::size_t y = 0; y < entityCompontent.size.y; ++y) {
            for (std::size_t z = 0; z < entityCompontent.size.z; ++z) {
                ...
                auto voxel =
                    entityCompontent
                        .voxelData[x + y * size.x + z * size.x * size.y];
                if (voxel == 0) {
                    continue;
                }
                for (std::size_t sx = 0; sx < scale; ++sx) {
                    for (std::size_t sy = 0; sy < scale; ++sy) {
                        for (std::size_t sz = 0; sz < scale; ++sz) {
                            auto voxelPosition = entityVoxelPos +
                                                 glm::ivec3(x, y, z) * scale +
                                                 glm::ivec3(sx, sy, sz);
